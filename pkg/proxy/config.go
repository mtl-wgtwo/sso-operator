...
const proxyConfigTemplate_pre6 = `
...
func renderConfig(config *Config, oldConfig bool) (string, error) {
	tmpl := template.New("oauth2_proxy.tpl")
	var err error

	if oldConfig {
		tmpl, err = tmpl.Parse(proxyConfigTemplate_pre6)
	} else {
